...
function buildNoResultMessage(queryText) {
  return `未能找到“${queryText}”对应的信息`;
}
  ...
  try {
    ...
    while (true) {
      ...
      for (const evt of events) {
        ...
        // ✅ 表格结果
          ...
          if (data.data.length === 0) {
            messages.value.push({
              role: "assistant",
              type: "text",
              content: buildNoResultMessage(q),
            });
          } else {
            messages.value.push({
              role: "assistant",
        ...
        // ✅ 后端错误信息不直接透出，避免暴露 SQL、表字段、栈信息等内部细节
