...
async def filter_table_info(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    writer({"type": "progress", "step": "筛选表信息", "status": "running"})
    ...
    try:
        ...
        writer({"type": "progress", "step": "筛选表信息", "status": "success"})
        logger.info(f"过滤后的表信息:{[filter_table_info['name'] for filter_table_info in filter_table_infos]}")

    except Exception as e:
        logger.error(f"筛选表信息失败：{e}")
        writer({"type": "progress", "step": "筛选表信息", "status": "error"})
        raise
